...
class MyApp:
                ...
                for user in allusers:
                    if usr == user['mac']:
                        if self.config['LOG_TO_FILE']:
                          self.append_line_to_file(self.config['LOG_TO_FILE'], str(datetime.now()) + ': ' + str(user))
                        new_users.append(user)
                        self.logger.info('%20s | %-30s  %-40s', user.get('mac'), user.get('hostname'), user.get('name'))
    ...
    def fetch_user_list(self):
        self.logger.debug('Fetch users from host %s:%d', self.config['UNIFI_HOST'], self.config['UNIFI_PORT'])
    ...
    def log_users(self, allusers):
        ...
        for user in allusers:
            self.logger.debug('%20s | %-30s  %-40s', user.get('mac'), user.get('hostname'), user.get('name'))
